...
namespace PuzzleAttack.Grid
{
    ...
    public class BlockSlipManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Convert world Y position to grid Y coordinate.
        /// </summary>
        private float WorldToGridY(float worldY, float gridOffset)
        {
            return (worldY - _gridManager.GridOrigin.y - gridOffset) / _tileSize;
        }

        /// <summary>
        /// Convert grid Y coordinate to world Y position.
        /// </summary>
        private float GridToWorldY(float gridY, float gridOffset)
        {
            return _gridManager.GridOrigin.y + gridY * _tileSize + gridOffset;
        }

        /// <summary>
        /// Get world position for a grid coordinate.
        /// </summary>
        private Vector3 GetWorldPosition(int gridX, int gridY, float gridOffset)
        {
            return _gridManager.GridToWorldPosition(gridX, gridY, gridOffset);
        }

        /// <summary>
        /// Get world position for a grid coordinate (float version for lerping).
        /// </summary>
        private Vector3 GetWorldPosition(float gridX, float gridY, float gridOffset)
        {
            return new Vector3(
                _gridManager.GridOrigin.x + gridX * _tileSize,
                _gridManager.GridOrigin.y + gridY * _tileSize + gridOffset,
                _gridManager.GridOrigin.z
            );
        }
        ...
        private void DrawBlockSlipGizmos()
        {
            ...
            // Use grid origin for positioning
            var gridOrigin = _gridManager != null ? _gridManager.GridOrigin : Vector3.zero;
            var startX = gridOrigin.x - lineWidth * 0.5f + (gridWidthVal * _tileSize * 0.5f);
            ...
            if (_showAllRows)
            {
                ...
                {
                    DrawRowThresholds(row, offsetY, startX, lineWidth, row == cursorRow ? 1f : 0.3f, gridOrigin.y);
                }
            }
            else
            {
                DrawRowThresholds(cursorRow, offsetY, startX, lineWidth, 1f, gridOrigin.y);
            }

            DrawLegend(offsetY, gridOrigin);
        }

        private void DrawRowThresholds(int row, float offsetY, float startX, float lineWidth, float alphaMultiplier, float gridOriginY)
        {
            var rowBottomY = gridOriginY + row * _tileSize + offsetY;
        ...
        private void DrawLegend(float offsetY, Vector3 gridOrigin)
        {
#if UNITY_EDITOR
            var legendPos = new Vector3(gridOrigin.x - 2f * _tileSize, gridOrigin.y + (_gridHeight + 1) * _tileSize + offsetY, 0);
        ...
        public bool IsBlockSlipTooLate(int columnX, int rowY)
        {
            ...
            var offsetY = _gridRiser?.CurrentGridOffset ?? 0f;
            var swapRowWorldY = GridToWorldY(rowY, offsetY);
        ...
        public bool TrySwapWithFallingTile(Vector2Int cursorPos)
        {
            ...
            foreach (var tile in _droppingTiles)
            {
                ...
                var tileWorldY = tile.transform.position.y;
                var offsetY = _gridRiser?.CurrentGridOffset ?? 0f;
                var rowBottomY = GridToWorldY(y, offsetY);
                var rowTopY = rowBottomY + _tileSize;
            ...
            Vector2Int idlePos, Vector2Int fallingPos)
        {
            ...
            var offsetY = _gridRiser?.CurrentGridOffset ?? 0f;
            var swapRowWorldY = GridToWorldY(swapRow, offsetY);
            ...
            foreach (var (tile, newPos) in newPositionsAbove)
            {
                ...
                var fromRow = Mathf.RoundToInt(WorldToGridY(currentWorldPos.y, offsetY));
            ...
            if (idleTile != null)
            {
                ...
                idleTile.transform.position = GetWorldPosition(fallingCol, swapRow, finalOffsetY);
            }
            ...
            if (fallingTile != null)
            {
                ...
                fallingTile.transform.position = GetWorldPosition(idlePos.x, idlePos.y, finalOffsetY);
            }
        ...
        private bool FindFallingBlockInColumn(int columnX, int rowY, out GameObject fallingBlock)
        {
            ...
            foreach (var tile in _droppingTiles)
            {
                ...
                var currentGridY = Mathf.RoundToInt(WorldToGridY(currentWorldY, offsetY));
        ...
        private IEnumerator ExecuteBlockSlip(GameObject swappingBlock, GameObject fallingBlock, Vector2Int slipPos)
        {
            ...
            var offsetY = _gridRiser.CurrentGridOffset;
            var swapRowWorldY = GridToWorldY(row, offsetY);
            ...
            foreach (var t in _droppingTiles)
            {
                if (t != null && t != swappingBlock && _dropTargets.TryGetValue(t, out var target) && target.x == col)
                {
                    ...
                    _dropAnimVersions[t] = GetVersion(t) + 1;
                }
            }
            ...
            foreach (var t in _droppingTiles)
            {
                ...
                var currentGridY = Mathf.RoundToInt(WorldToGridY(tileWorldY, offsetY));
            ...
            foreach (var (tile, oldTarget, newTarget) in retargetPlan)
            {
                ...
                var currentPos = tile.transform.position;
                var targetWorldY = GridToWorldY(newTarget.y, offsetY);
            ...
            if (swappingBlock != null)
            {
                ...
                ts?.FinishMovement();
                swappingBlock.transform.position = GetWorldPosition(col, row, _gridRiser.CurrentGridOffset);
            }
        ...
        private IEnumerator AnimateSwap(GameObject tile, Vector2Int targetPos)
        {
            ...
            var offsetY = _gridRiser.CurrentGridOffset;
            var startGridX = (startWorldPos.x - _gridManager.GridOrigin.x) / _tileSize;
            var startGridY = WorldToGridY(startWorldPos.y, offsetY);
            var currentTarget = targetPos;
            var targetGridY = (float)currentTarget.y;

            var previousOffset = offsetY;
            var elapsed = 0f;
            ...
            while (elapsed < _swapDuration)
            {
                ...
                tile.transform.position = GetWorldPosition(currentGridX, currentGridY, currentOffset);

                elapsed += Time.deltaTime;
                yield return null;
            }

            tile.transform.position = GetWorldPosition(currentTarget.x, currentTarget.y, _gridRiser.CurrentGridOffset);
        ...
        private IEnumerator AnimateCascadeSmooth(GameObject tile, Vector2Int toPos)
        {
            ...
            try
            {
                while (elapsed < duration && tile != null)
                {
                    ...
                    tile.transform.position = GetWorldPosition(currentTarget.x, currentGridY, currentOffset);
                    elapsed += Time.deltaTime;
                    yield return null;
                }
        ...
        private IEnumerator AnimateCascadeQuick(GameObject tile, Vector2Int toPos)
        {
            ...
            var distance = Mathf.Abs(targetGridY - startGridY);
            var duration = distance / _dropSpeed * 0.5f;
            var elapsed = 0f;
            var previousOffset = offsetY;
        ...
        private IEnumerator AnimateDrop(GameObject tile, Vector3 fromWorldPos, Vector2Int toPos, bool checkObstructions)
        {
            ...
            var offsetY = _gridRiser.CurrentGridOffset;
            var startGridY = WorldToGridY(fromWorldPos.y, offsetY);
            var currentTarget = toPos;
            ...
            try
            {
                while (elapsed < duration && tile != null && _droppingTiles.Contains(tile))
                {
                    ...
                    {
                        var currentWorldY = tile.transform.position.y;
                        var targetWorldY = GridToWorldY(currentTarget.y, currentOffset);
                        ...
                        if (distanceToTarget > 0.5f)
                        {
                            ...
                            if (newTarget.HasValue && newTarget.Value.y != currentTarget.y)
                            {
                                ...
                                if (atNewTarget == null || atNewTarget == tile)
                                {
                                    ...
                                    var retargetGridY = WorldToGridY(tile.transform.position.y, currentOffset);
